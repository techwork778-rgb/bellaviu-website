...
const BookingsTable = () => {
  ...
  const filteredUsers = users.filter((user) => {
    const fullname = `${user.user_name} ${user.user_email} ${user.user_id}`.toLowerCase();
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedUsers(filteredUsers.map((user) => user.order_id)); // Select all users across all pages
    }
  ...
  const handleExport = () => {
    const usersToExport = users.filter((user) =>
      selectedUsers.includes(user.order_id)
    );
  ...
  return (
    ...
    <Box sx={{ p: 3, bgcolor: "background.paper", borderRadius: 2 }}>
      <div className="d-flex flex-column" style={{ marginBottom: "10px" }}>
        <h3 className="title">All Booking</h3>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ whiteSpace: "nowrap" }}>Property Id</TableCell>
              <TableCell sx={{ whiteSpace: "nowrap" }}>Property Name</TableCell>
            ...
            {filteredUsers.length === 0 ? (
              ...
              paginatedData.map((user, index) => (
                <TableRow key={user.user_id} hover>
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={selectedUsers.includes(user.order_id)}
                      onChange={() => handleSelectUser(user.order_id)}
                    />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {new Date(user.check_in).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  <TableCell>
                    {new Date(user.check_out).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
                  ...
                  <TableCell>{user.property_name}</TableCell>
                  <TableCell>{user.price}</TableCell>
                 <TableCell>
  {user.uploaded_file?.endsWith(".pdf") ? (
    <a href={user.uploaded_file} target="_blank" rel="noopener noreferrer">
      View PDF
    </a>
  ) : (
    <img
