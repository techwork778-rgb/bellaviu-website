...
const BookingsTable = () => {
  ...
  const fetchContacts = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/get_career", {
        method: "GET",
      });
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedUsers(filteredCustomer.map((user) => user.id)); // Select all rows of current page
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch("/api/delete_carrer", {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ userIds: selectedUsers }), // Send array of user IDs
      });
  
      const data = await response.json();
  
      if (response.ok) {
        toast.success("Selected career applications deleted successfully.");
  ...
  return (
    <Box sx={{ p: 3, bgcolor: "background.paper", borderRadius: 2 }}>
      <div className="d-flex flex-column" style={{ marginBottom: "20px" }}>
        <h3 className="title">All Career List</h3>
                ...
                <TableRow key={user.contact_us_id} hover>
                  <TableCell padding="checkbox">
                    <Checkbox
                      ...
                      onChange={() => handleSelectUser(user.id)} // Call handleSelectUser on change
                    />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {user.resume_path ? (
                      <Button
                        ...
                        onClick={() =>
                          window.open(
                            `/api/get-file?fileName=${user.resume_path}`,
                            "_blank"
                          )
                        }
                      >
